class Solution {
    private final int MOD = 1000000007; //  Super important number! Like the "secret sauce" for calculations. Prevents numbers from getting too HUGE and messing things up.

    private static final int MAX_SIZE = 100000; //  How big can our arrays be? This sets the limit.
    private static final int[] FACTORIALS = new int[MAX_SIZE]; //  This array stores factorials (1*2*3*...*n) to speed up calculations. Like having a cheat sheet.

    public int countGoodArrays(int n, int m, int k) {
        // n = length of array, m = possible numbers, k = times the next number changes

        if (FACTORIALS[0] == 0) { //  Have we calculated factorials before? (Optimization!)
        ...
        return (int) result; // Boom!  Here's the answer, cast to integer type.
    }

    // Power Function : Calculates base raised to exponent (base^exponent) modulo MOD 
    ...
    // Combination Function: Calculates C(a, b)  combinations modulo MOD
    ...
    // Factorial Function: Calculates factorial of a number  a! 
    ...
    // Modular Inverse Function: Finds the modular inverse of a number modulo MOD 
